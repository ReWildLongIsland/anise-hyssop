"""
One-time script to populate the Teams Config Google Sheet.
...
ws = client.open_by_key(TEAMS_SHEET_ID).sheet1

# Clear existing data
ws.clear()

# Headers
headers = ["TeamID", "Name", "Type", "Description", "ZipCodes"]
...
# Write all data, forcing text format for ZipCodes column
ws.update(values=rows, range_name="A1", value_input_option="RAW")

print(f"Done! Wrote {len(rows) - 1} teams to sheet {TEAMS_SHEET_ID}")
