---
name: start-bumblebee
description: Start a work session on Bumblebee. Pulls latest from GitHub and reviews PLAN.md so you know exactly where to pick up.
---

Pull the latest code from GitHub and orient for the session.

**Git pull:**
```!
cd "$(git -C "C:/Users/raju/Desktop/Code/Anise Hyssop" rev-parse --show-toplevel 2>/dev/null || echo "C:/Users/raju/Desktop/Code/Anise Hyssop")" && git pull
```

**Local changes (if any):**
```!
cd "$(git -C "C:/Users/raju/Desktop/Code/Anise Hyssop" rev-parse --show-toplevel 2>/dev/null || echo "C:/Users/raju/Desktop/Code/Anise Hyssop")" && git status --short
```

**Recent commits:**
```!
cd "$(git -C "C:/Users/raju/Desktop/Code/Anise Hyssop" rev-parse --show-toplevel 2>/dev/null || echo "C:/Users/raju/Desktop/Code/Anise Hyssop")" && git log --oneline -5
```

**PLAN.md:**
```!
cat "C:/Users/raju/Desktop/Code/Anise Hyssop/PLAN.md"
```

Based on all of the above, please:
1. Summarise what was pulled (new commits, key file changes) — or confirm already up to date
2. State the current active phase and its status from the Quick Status table
3. List the exact next steps from PLAN.md for the current phase
4. Flag any uncommitted local changes that need attention before starting
5. Confirm the session is ready and tell me what to work on first

> **Bumblebee runtime note:** Backend runs on port **8001**. Vite proxy in `vite.config.ts` points to `http://localhost:8001`. Start the backend with:
> `uvicorn app.main:app --reload --port 8001`
